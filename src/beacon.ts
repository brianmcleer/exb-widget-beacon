/*
  beacon.ts  -  anonymous usage and error telemetry for ArcGIS Experience Builder custom widgets.
  https://github.com/brianmcleer/exb-widget-beacon   (Apache-2.0)

  Drop this file into a widget at src/shared/beacon.ts. If you maintain several widgets, keep one
  master copy and copy it into each widget unchanged; never edit a widget's copy.
  ...
  (layer 0). No sink item means telemetry is off: an organization that installs a widget with
  this file sends nothing anywhere unless it publishes its own sink (setup/beacon_sink_setup.py).
  The lookup result is cached in sessionStorage for the page session.
  ...
  Esri's own widgets are tracked as well. Once any widget has called init, the module watches
  the app store: every out-of-the-box widget in the app (uri widgets/common/..., widgets/arcgis/...,
  widgets/layout/...) gets one "loaded" row per page load, and an "open" row each time its
  runtime state becomes OPENED (widget controller, sidebar, and so on). widget_name is Esri's
  manifest name (legend, print, map-layers, ...) and widget_version is the Experience Builder
  version. An app with no instrumented widget runs none of this code.
